...
var _Sources = (() => {
  ...
  function rotateCDN(url, attempt) {
    try {
      const u = new URL(url);
  ...
  var BatoToInfo = {
    ...
    sourceTags: [{ text: "Multi Language", type: import_types.BadgeColor.BLUE }],
  ...
  var BatoTo = class {
    ...
    getMangaShareUrl(id) {
      return `${BATO_DOMAIN}/series/${id}`;
    }
    async getMangaDetails(id) {
      const r = await this.requestManager.schedule(App.createRequest({
        url: `${BATO_DOMAIN}/series/${id}`,
        method: "GET"
      }), 1);
      return parseMangaDetails(this.cheerio.load(r.data), id);
    }
    async getChapters(id) {
      ...
      return parseChapterList(this.cheerio.load(r.data), id);
    }
    async getChapterDetails(mid, cid) {
      const r = await this.requestManager.schedule(App.createRequest({
        url: `${BATO_DOMAIN}/chapter/${cid}`,
        method: "GET"
      }), 1);
      return parseChapterDetails(this.cheerio.load(r.data), mid, cid);
    }
    async getHomePageSections(cb) {
      const r = await this.requestManager.schedule(App.createRequest({ url: BATO_DOMAIN, method: "GET" }), 1);
      parseHomeSections(this.cheerio.load(r.data), cb);
    }
    async getViewMoreItems(id, meta) {
      const page = meta?.page ?? 1;
      const r = await this.requestManager.schedule(App.createRequest({
        url: `${BATO_DOMAIN}/browse?...`,
        method: "GET"
      }), 1);
      const $2 = this.cheerio.load(r.data);
      return App.createPagedResults({ results: parseViewMore($2), metadata: !isLastPage($2) ? { page: page + 1 } : void 0 });
    }
    async getSearchResults(q, meta) {
      const r = await this.requestManager.schedule(App.createRequest({
        url: `${BATO_DOMAIN}/search?word=${encodeURI(q.title ?? "")}`,
        method: "GET"
      }), 1);
      const $2 = this.cheerio.load(r.data);
      return App.createPagedResults({ results: parseSearch($2), metadata: void 0 });
    }
    async getSearchTags() {
      return parseTags();
    }
