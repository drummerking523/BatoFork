...
var _Sources = (() => {
  ...
  function getNextCDN(url, attempt) {
    try {
      ...
      const nextHost = CDN_HOST_POOL2[attempt % CDN_HOST_POOL2.length];
  ...
  var BatoTo = class _BatoTo {
      ...
      this.requestManager = App.createRequestManager({
        ...
        interceptor: {
          ...
          interceptResponse: async (response) => {
            ...
            const transientStatus = status === 503 || status === 403 || status === 0 || status === -1001 || status === -1003;
            const shouldRetry = isBatoImage && transientStatus && attempt < CDN_HOST_POOL2.length;
            if (shouldRetry) {
    ...
    async getChapters(mangaId2) {
      ...
      this.CloudFlareError(response.status);
      const $2 = this.cheerio.load(response.data);
      const chapters = parseChapterList($2, mangaId2);
