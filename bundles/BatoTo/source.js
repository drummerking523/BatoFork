...
var _Sources = (() => {
  ...
  var buildCdnFallbacks = (originalUrl) => {
    try {
  ...
  var parseMangaDetails = ($2, mangaId2) => {
    ...
    const author = authorElement.length ? authorElement.children().map((_, e) => $2(e).text().trim()).toArray().join(", ") : "";
    const artistElement = $2('div.attr-item b:contains("Artists")').next("span");
    const artist = artistElement.length ? artistElement.children().map((_, e) => $2(e).text().trim()).toArray().join(", ") : "";
    const arrayTags = [];
    ...
    const status = rawStatus === "COMPLETED" ? "Completed" : rawStatus === "HIATUS" ? "Hiatus" : "Ongoing";
  ...
  var parseTags = () => [];
  var parseSearch = ($2) => [];
  var isLastPage = ($2) => $2(".page-item").last().hasClass("disabled");
  ...
  function getNextCDN(url, attempt) {
    try {
      const u = new URL(url);
      if (!u.hostname.includes("mbwww.org")) return url;
      const next = CDN_HOST_POOL2[attempt % CDN_HOST_POOL2.length];
  ...
  async function retryWithBackoff(fn, retries = 4, delay = 400) {
    try {
      return await fn();
    } catch (err) {
  ...
  var BatoTo = class _BatoTo {
      ...
      this.requestManager = App.createRequestManager({
        ...
        interceptor: {
          ...
          interceptResponse: async (response) => {
            ...
            const shouldRetry = url.includes("mbwww.org") && (status === 503 || status === 403 || status === 0 || status === -1001 || status === -1003);
            if (shouldRetry && attempt < CDN_HOST_POOL2.length) {
    ...
    async getMangaDetails(mangaId2) {
      const res = await this.requestManager.schedule(
        App.createRequest({ url: `${BATO_DOMAIN}/series/${mangaId2}`, method: "GET" }),
        1
      );
      this.CloudFlareError(res.status);
      return parseMangaDetails(this.cheerio.load(res.data), mangaId2);
    }
    async getChapters(mangaId2) {
      ...
      this.CloudFlareError(res.status);
      const chapters = parseChapterList(this.cheerio.load(res.data), mangaId2);
      if (!chapters.length) return [];
      return chapters;
    }
    async getChapterDetails(mangaId2, chapterId2) {
      const res = await this.requestManager.schedule(
        App.createRequest({ url: `${BATO_DOMAIN}/chapter/${chapterId2}`, method: "GET" }),
        1
      );
      this.CloudFlareError(res.status);
      return parseChapterDetails(this.cheerio.load(res.data), mangaId2, chapterId2);
    }
    async getHomePageSections(cb) {
      const res = await this.requestManager.schedule(
        App.createRequest({ url: `${BATO_DOMAIN}`, method: "GET" }),
        1
      );
      this.CloudFlareError(res.status);
      parseHomeSections(this.cheerio.load(res.data), cb);
    }
    async getViewMoreItems(id, metadata) {
      const page = metadata?.page ?? 1;
      let param = id === "popular_updates" ? `?sort=views_d.za&page=${page}` : `?sort=update.za&page=${page}`;
      ...
      const res = await this.requestManager.schedule(
        App.createRequest({ url: `${BATO_DOMAIN}/browse`, method: "GET", param }),
        1
      );
      const $2 = this.cheerio.load(res.data);
      metadata = !isLastPage($2) ? { page: page + 1 } : void 0;
      return App.createPagedResults({ results: parseViewMore($2), metadata });
    }
    async getSearchResults(query, metadata) {
      const page = metadata?.page ?? 1;
      const req = query.title ? App.createRequest({ url: `${BATO_DOMAIN}/search?word=${encodeURI(query.title)}&page=${page}`, method: "GET" }) : App.createRequest({ url: `${BATO_DOMAIN}/browse?genres=${query?.includedTags?.[0]?.id}&page=${page}`, method: "GET" });
      ...
      return App.createPagedResults({
        results: parseSearch($2, langSearchFilter, langs),
        metadata
      });
    }
    async getSearchTags() {
